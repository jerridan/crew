...
A lead session takes the same two flags, and it starts each project-lead session itself. Run it inside tmux: `/crew:lead` opens one tmux window per goal. In iTerm2, set `CREW_LAUNCH=iterm2` and it opens one native tab per goal instead (design §15.89). Start it from a directory you have opened in Claude Code before, and answer the folder-trust dialog if one appears — a lead cannot restart itself past that dialog (design §15.74j).
