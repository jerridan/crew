...
A band also sets the review a package gets: a `light` package skips the plan
gate, and can skip the deliverable review. Every band keeps the spec critic
and the package review, and the record names each skipped step (design
§15.77).
...
|---|---|---|
| Lead | Holds a portfolio of goals: writes a charter for each, starts one project-lead session per goal, answers what its charters and records settle, and brings you the rest in one batch. | yes |
