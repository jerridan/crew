...
**Pick the path at step 1, from the charter.** The test is the acceptance
criterion: a goal whose criterion cannot be written until something is
diagnosed is a symptom, not a change. That question is already asked at step
1, so this costs no extra turn.
...
that only says "passes after" is satisfied by a test that never failed, which
is the defect §7 exists to catch.

The project lead writes the reproduction before it diagnoses anything. When
it cannot make the symptom happen on demand, it has no falsifiable criterion,
§6 trigger 1 fires, and the run stops before it spends anything. "Gather more
data" is the escalation, not a hypothesis.
...
convenes a council (§6.1) with three assigned advocates. This is the second of
the two cases that still earn a full council: competing root causes over one
body of evidence is what assigned positions are for. One surviving hypothesis
is not a council. The project lead tests it under Phase 3 and moves on.
...
principal asked to have answered rather than fixed. Such a run has one
deliverable and no package.
...
| `systematic-debugging` Phase 4.5: after 3 failed fixes, "discuss with your human partner" | The fix-round breaker and escalation trigger 6 (sections 9.2, 6) | Same stop, reached by a rule instead of a conversation |
...
55. **The investigation path is designed, and five choices in it were not
    ...
    **The path is chosen at step 1, not at step 5.** §9.1's table is the
    shape choice and it runs after scouting. This choice has to come before
    scouting, because it changes what the scouts are asked. The test is the
    one step 1 already applies: a goal whose acceptance criterion cannot be
    written until something is diagnosed is a symptom. So the branch costs no
    new step, and §9.1 gets one pointer rather than a fourth row.
