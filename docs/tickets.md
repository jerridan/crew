...
rule under The preference sweep, `record-format.md` owns the entry that
records the sweep, and `crew-record.py` gained `escalation add` so a batch
appends instead of replacing the list (design §6.3, §15.51). The ticket
stays open for the two runs
