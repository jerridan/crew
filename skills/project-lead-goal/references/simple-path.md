...
## What other files cite by number

Every rule below carries one number from the loop other files cite.
...
| step 8 | The IC |
| step 9 | The evidence |
| step 10 | The package review |
| step 11 | The fix rounds |
| step 12 | Integration |
| step 13 | The deliverable review |
| step 14 | The end of the run |

**A bounded edit runs a subset.** `SKILL.md`'s shape table sends it here for
the split and the branch, then integration, the deliverable review and the
end of the run. You make the edit yourself, so there is no IC to dispatch and
no package review to run. Every other rule holds as written, with the three
exceptions the split, the branch and the deliverable review name.
...
`crew:deliverable-reviewer` reads it at the deliverable review.

A bounded edit writes the same file and the same entry, so that integration
and the deliverable review have a package to mark and a split to read. It
...
`record-format.md` keeps those out of every file set, and integration is
where you edit them.
...
One **unnamed** subagent at the package's band model does the work:
...
full text, the brief, the file set, this checkout's path, the interface
contract, the acceptance criterion, the global constraints section, the
record root, the package id, and **that it is a subagent** —
...
else — this is the principal's own checkout, and `checkout_restored` at the
end of the run records what it was left on.
...
**Every round goes back through the evidence and the package review** — a fix
nobody re-reviewed is a claim. Leave only on `Verdict: accepted`. Increment
`fix_rounds_used` **before** the round runs — the package review and the
round name their files from it, so a late increment overwrites the previous
round's files. Five is the cap: at it, fix the package yourself or park it as
...
package review's diff predates the fix rounds and the shared-file edits you
just made, which the next reviewer's shared-file check exists to read.
...
seven checks need the record. Adjudicate as `SKILL.md`'s "How you adjudicate
a review" says; clear every `[Critical]` first.
...
preference sweep writes it only when `full-path.md`'s launch check 3 failed,
