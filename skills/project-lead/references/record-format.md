...
This file owns the goal record: one directory per goal, written by the
project lead.
...
| `nudges_used` | integer, capped at one per dispatch (`full-path.md`'s "The idle nudge"). Counts the current dispatch only, so every re-dispatch of the package resets it to 0. Persisted because a resumed session holds no memory of a nudge it already sent. With `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` set, `SendMessage` reaches the simple path's named IC by name like any other teammate; unset, the same dispatch is a plain subagent with no message channel, and the simple path leaves this field 0 either way, because full-path.md's "The idle nudge" is a full-path step. |
| `ic_name` | the name of the agent assigned to this package, written before every dispatch on every path. On the full path it cross-references `worktrees.json`, which maps this name to a worktree path; without it, nothing maps a package back to the worktree that must verify it. On the simple path there is no per-IC `worktrees.json` entry (`simple-path.md`, "Create the branch"), but the field still holds the current dispatch's name, so a respawn reads it and takes the next `r<n>` instead of reusing one. A respawn — a fix round, the next package, or recovery from a crash — gives the IC a new name, and the project lead writes that name here (and, on the full path, into `worktrees.json`'s entry for the same worktree in the same turn) so nothing ever names two different ICs the same. |
...
- `run_state` values `active`, `blocked`, `delivered`, `interrupted`, `complete` — consumer: this file's `run_state` transitions table; both crew hooks; stage 5, stage 6
