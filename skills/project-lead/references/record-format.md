...
| `principal` | who to send an escalation to, when the goal did not arrive in this session. Set it with `run set principal '"<name>"'` from the `from` attribute of the `<cross-session-message>` that carried the goal (`autonomy-contract.md`). Absent when a human typed the goal in this session, and a `--resume` session that finds it absent escalates in its own pane. |
