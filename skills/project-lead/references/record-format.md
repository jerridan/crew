...
| `steps_skipped` | a list of `{step, package, reason, at}`, one entry per step a band let the run skip. `step` is `plan-gate` or `deliverable-review`; `package` is the package id for `plan-gate` and the deliverable id for `deliverable-review`; `reason` is one line naming the band and the conditions that held. `band-rubric.md`'s "What a band skips" decides what may go in here, and nothing else may. Absent until the first skip, which is what makes an absent field mean "every step ran". Write it with `run set steps_skipped <json>`, the whole list each time. |
