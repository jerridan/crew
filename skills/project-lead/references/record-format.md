...
| `state` | one of `pending`, `in-flight`, `draft-pr-opened`, `work-complete`, `abandoned`. `draft-pr-opened` and `work-complete` are a deliverable's own terminal states, not `integrated` — design §9.3 and §11 stop at opening a PR, so no crew state ever means a deliverable reached `main`. `draft-pr-opened` also names a PR that opened ready for review. The name is older than that default, and past records use it (design §15.99). `work-complete` means the work is complete and trusted but no PR was opened; `abandoned` means the work is not trusted. |
